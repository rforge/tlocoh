...
#'
#' Returns a subset of lxy based on ptid, idx, or dates
#'
...
#'
#' @export
#' @seealso \code{\link{xyt.lxy}}, \code{\link{lxy.repair}}
...
lxy.subset <- function(lxy, id=NULL, ptid=NULL, idx=NULL, dt.start=NULL, dt.end=NULL, dt.int.round.to=0.1, tau.diff.max=0.02) {
    ...
    if (length(idx.filter) == num.pts) {
        ...
        rwp.dti.lst <- xyt.rw.params.dt.int(id=lxy[["pts"]][["id"]], xys=coordinates(lxy[["pts"]]), dt=lxy[["pts"]][["dt"]], dt.int.round.to=dt.int.round.to, tau.diff.max=tau.diff.max) 
