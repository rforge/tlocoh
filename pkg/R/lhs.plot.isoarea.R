#' Plot the area of isopleths as a function of the k/a/r parameter
#'
...
lhs.plot.isoarea <- function(lhs, id=NULL, k=NULL, r=NULL, a=NULL, s=NULL, hs.names = NULL, sort.metric=NULL,
    ...
    for (immVal in unique(id.mode.metric)) {
         ...
         fn.series <- paste(".", param.str, ".vs.isoarea", sep="")
          ...
          if (title.show) {
              if (is.null(title)) {
                  ...
                  title.use <- paste(param.str, " vs. isopleth area", hs.sub, sep="")
              } else {
                  title.use <- title
              }
          } else {
         ...
         if (legend) legend("topleft", as.character(iso.levels.immVal), col=col.overlay, lty=1, title="iso level", cex=0.8)
