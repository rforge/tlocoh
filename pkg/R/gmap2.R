#' Download a static background image from Google
#'
#' @export

gmap2 <- function(x, exp=1, type='terrain', filename='', style=NULL, scale=1, zoom=NULL, size=c(640, 640), rgb=FALSE, lonlat=FALSE, google_apikey=NULL, ...) {
...
#
# March 2019
# Updated by Andy Lyons - added an argument to pass a key for the Google Static Maps API (required as of 2018)

	if (!requireNamespace('rgdal') ) {stop('rgdal not available')}
  if (!requireNamespace('raster') ) {stop('raster not available')}
 ...
 if (is.null(google_apikey)) {
   stop("google_apikey is required. For more info, see https://developers.google.com/maps/documentation/maps-static/get-api-key")
 }
...
#	gurl <- "http://maps.google.com/staticmap?"
	...
	if (is.character(x)) {
    stop("Can not geocode, sorry")
		x <- geocode(x, oneRecord=TRUE)
		if (is.na(x$latitude)) { 
			stop('location not found') 
		}
		x <- raster::extent(as.vector(as.matrix(x[5:8])))

	} else {
		prj <- raster::projection(x, asText=TRUE)
		
		if (raster::isLonLat(prj) ) {
			...
			if (is.na(prj) ) {
				...
				# warning('CRS is unknown, and does not look like Lon/Lat, assuming it is Mercator')
					...
					p[, 1] <- p[, 1]/rad
					p <- p / (pi/180)
					x <- raster::extent(p[1,1], p[2,1], p[1,2], p[2,2])
				}
			} else {
				x <- raster::extent(raster::projectExtent(x, "+proj=longlat +datum=WGS84") )
			}
		}
	}
		e <- x * exp
